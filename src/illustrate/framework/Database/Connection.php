...
class Connection implements Serializable
{
	...
	public function applyMigrations()
	{
		...
		$newMigrations = [];
		$files = scandir(Helpers::config('migrations', 'database'));
		$toApplyMigrations = array_diff($files , $appliedMigrations);
		foreach ($toApplyMigrations as $migration) {
			if ($migration === '.' || $migration === '..') {
				continue;
			}
			require_once Helpers::config('migrations', 'database') . $migration;
		...
		if (!empty($newMigrations)) {
			...
			$this->log("There are no migrations to apply",$migration);
		}
	}

	protected function createMigrationsTable()
	{
		$this->getPDO()->exec("CREATE TABLE IF NOT EXISTS migrations (
	...
	{
		$statement = $this->getPDO()->prepare("SELECT migration FROM migrations");
		$statement->execute();
	...
	protected function saveMigrations(array $newMigrations)
	{
		...
		$statement = $this->getPDO()->prepare("INSERT INTO migrations (migration) VALUES 
            $str
        ");
		$statement->execute();
	}
	protected $input;
	protected $output;
	public function setLog($input,$output){
		$this->input = $input;
		$this->output = $output;
	}
	private function log($message,$migration)
	{
		$io = new SymfonyStyle($this->input,$this->output);
		$io->writeln('<fg=green>['.date("Y-m-d H:i:s").']</> <fg=blue>'.$message.'</> <fg=yellow>'.$migration.'</>'.PHP_EOL);
	}
}